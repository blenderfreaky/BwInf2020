...
\begin{description}
    \item[Romino] Eine Ansammlung von Blöcken auf einem 2D Integer-Gitter nach den Regeln der Aufgabe.
    
    \item[Block] Eine Position auf dem 2D Gitter, die einem Romino angehört.

    \item[Blocksadditionsmöglichkeit] Eine Blocksadditionsmöglichkeit eines Rominos ist eine Position im 2d-Gitter, wo in zukünftigen Schritten ein neuer Block angefügt werden kann.

    In den Grafiken als grauer Kreis dargestellt.

    \item[(echte) Diagonale] Die (echte) Diagonale ist die in der Aufgabenstellung beschriebene diagonale Verbindung zweier Blöcke mit zwei fehlenden Blöcken.
    
    \item[geschützte Diagonale] Die geschützte Diagonale ist die echte Diagonale die immer geschützt wird.

    In den Grafiken sind die von der geschützten Diagonale geschützten Blöcke als kleine weiße/schwarze Punkte dargestellt.
\end{description}
...
Nehme man beispielsweise das 2er-Romino, kann man zum Finden aller \(3 (= 2 + 1)\)er - Rominos wie folgt Blöcke anfügen:
...
Somit ergeben sich folgende 3er-Rominos:
...
Bei den möglichen Blockadditionen beim 2er-Romino beispielsweise würden hierfür die für die Diagonale relevanten Blöcke aus den Blockadditionsmöglichkeiten entfernt:
...
Im folgenden wird ein Beispiel für die Schritte fortgeführt.
Die Ausganssituation des Beispiels ist wie folgt:
...
Andersgesagt setze man die \(((y * <\text{Anzahl an Blöcken}>) + x)\)-te Ziffer des Wertes als Binärzahl auf Eins.
Diese Formel ergibt sich, da sie für jede Position im Gitter einen eindeutigen Wert hat. Hierbei kann es nicht sein, dass eine Kollision enstehen kann, da ein Romino nicht aus einem Gitter der Größe \(<\text{Anzahl an Blöcken}> \times <\text{Anzahl an Blöcken}>\) herauswachsen kann.
...
Weiterdem überlädt |BitBuffer512|
...
Zahlentyp, |ulong|, bereits mit 8er-Rominos komplett
...
Ergebnisse für |Rominos.CLI --size 5 --latex --highlightPossibleExtensions --highlightDiagonalBlockade|

\subsubsection{Rominos mit Größe 2}

Es gibt insgesamt 1 Romino mit Größe 2.
...
\subsubsection{Rominos mit Größe 3}

Es gibt insgesamt 3 Rominos mit Größe 3.
...
\subsubsection{Rominos mit Größe 4}

Es gibt insgesamt 17 Rominos mit Größe 4.
...
\subsubsection{Rominos mit Größe 5}

Es gibt insgesamt 82 Rominos mit Größe 5.
